...
export default function Edit() {
  ...
  const handleOnSubmit = async () => {
    if (title === "") {
      ...
      try {
        ...
        await axios
          .put(`/post/edit/${router.query.id}`, formData, {
            headers: {
  ...
  const handleOnChange = (e) => {
    ...
    if ([...reuploadImages, ...tmpArr].length + images.length < 5) {
      ...
      setIsModal(true);
      setReuploadImages(
        [...reuploadImages, ...tmpArr].slice(0, 5 - images.length)
      );
  ...
  return (
    <div className="post">
      ...
      <div className="postImages">
        {images !== undefined && images.every((i) => i !== null)
          ? images.map((i) => (
              <div className="postImages_wrap" key={i.id}>
                <button
                  onClick={() => setImages(images.filter((v) => i.id !== v.id))}
                >
                  X
                </button>
                <Image
        ...
        {preView?.map((image, id) => (
          <div className="postImages_wrap" key={id}>
            <button
              onClick={() => setPreView(preView.filter((i) => i !== image))}
            >
              X
            </button>
      ...
      <div className="postBtn">
        <button
          className="cancle"
          onClick={() => router.push(`/post/read/${postData.id}`)}
        >
          취소
        </button>
